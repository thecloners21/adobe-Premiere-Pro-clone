...
- **media.js** — import file (video/audio/immagini), `objectURL`, lettura metadati, thumbnail, decode audio per waveform; render del media bin; `runtime` (Map id→{element,thumb,audioBuffer}). **Nesting**: `createNestFromTimeline` collassa l'intera timeline in `project.nests[]` ({id,name,tracks}) e crea un media `kind:'sequence'` (con `nestId`, durata) posto come unica clip su V1; `rehydrateSequence` per il refresh; `sequenceThumb` placeholder bin.
- **Sequenze annidate (nesting)**: una clip `kind:'sequence'` referenzia un `nest` (mini-EDL). In preview, `renderNestFrame(seqMedia, nestT, depth)` compone le tracce del nest in un compositore offscreen del pool `nestComps[slot]` (slot incrementale `nestRenderIdx` azzerato a ogni frame, niente collisioni anche con sequenze affiancate/transizioni) e ne restituisce il canvas, usato come sorgente dalla clip esterna (con i suoi fx/grading/maschera). `drawClip`/`drawTransition` accettano `(cmp, depth)`; `composite(T)` estratto per testabilità. Audio appiattito da `collectAudioClips()` (ricorsivo: offset/speed cumulativi). Render server: le clip-sequenza sono **saltate** (nesting solo via export browser).
...
- **Secondaria HSL**: `clip.secondary` ({on,color,range,soft,satMin,dHue,dSat,dLum}, `defaultSecondary()`). `computeSecondary` deriva la tonalità chiave dal colore; shader (uniform `uSec*`, `rgb2hsv`/`hsv2rgb`) costruisce una maschera sulla banda di tonalità (distanza circolare + soglia saturazione) e applica hue-shift/saturazione/luminosità solo lì (`mix(orig, corr, mq)`). Cache `secFor` in preview, gruppo "Secondaria HSL" nell'inspector. Browser-only in export (come curve/LGG).
...
- **Color grading avanzato** (curve RGB, Lift/Gamma/Gain, Secondaria HSL): fedele in anteprima ed **export browser**; **non** applicato nel render server ffmpeg (che gestisce solo eq/hue/temp/blur ecc.). Usa l'export browser per il grading completo.
...
- **Nesting**: "Annida" collassa l'intera timeline in una sequenza (non c'è multi-selezione di clip singole); doppio-clic per entrare/modificare il nest non è ancora previsto (si modifica ri-annidando). Reso fedelmente in anteprima ed **export browser**; il render server ffmpeg salta le clip-sequenza. L'audio annidato è appiattito (offset/velocità cumulativi).
